...
def state_difficulty(state: State):
    ...
    cross_diff *= 1
    leg_match_diff *=20
    diff += center_diff + scrunched_up_diff + angle_diff + limb_strength_diff + separation_diff + leg_match_diff + cross_diff
        ...
        while not frontier.isEmpty():
            ...
            for next_state, action in cur_state.getStateSuccessors():
                if next_state not in explored: 
                    ...
                    next_state.costs.append(state_difficulty(next_state) + move_difficulty(cur_state, action[0], action[1]))
                    next_state.moves = copy(cur_state.moves)
                    next_state.moves.append(action)
                    if next_state in [tup[2] for tup in frontier.heap]:
                        frontier.update(next_state, self.get_cost_value(next_state.costs))
                    else:
